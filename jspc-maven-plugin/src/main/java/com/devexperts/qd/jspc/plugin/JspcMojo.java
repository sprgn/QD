 ...
 */
/*
 * Modified to support JPS pages mapped via servlet-mapping to jsp servlet.
 * by Roman Elizarov, Devexperts LLC, 2013.
 */

package com.devexperts.qd.jspc.plugin;
...
public class JspcMojo extends AbstractMojo
{
    ...
    /**
     * Webapp target directory. All compiled JSP files are copied into this directory to merge them with
     * other files that might have been generated by minification process, for example.
     * Introduced to support JPS pages mapped via servlet-mapping to jsp servlet.
     *
     * @parameter expression="${webappTargetDir}" default-value="${project.build.directory}/${project.build.finalName}"
     */
    private String webAppTargetDirectory;
    ...
    public void execute() throws MojoExecutionException, MojoFailureException
    {
        if (getLog().isDebugEnabled())
        {
            ...
            getLog().info("webAppTargetDirectory=" + webAppTargetDirectory);
        ...
        {
            prepare();
            String webXmlText = compile();
            cleanupSrcs();
            mergeWebXml(webXmlText);
        }
        catch (Exception e)
        {
    ...
    public String compile() throws Exception
    {
        ...
        jspc.setUriroot(webAppTargetDirectory); // files are copied into there by parseWebXmlAndGetJspFiles
        ...
        String webXmlText = parseWebXmlAndGetJspFiles(fileNames);
        String jspFiles = StringUtils.join(fileNames.toArray(new String[fileNames.size()]), ",");
    ...
    private String parseWebXmlAndGetJspFiles(Set<String> jspFilePaths)
        throws ParserConfigurationException, IOException, SAXException, TransformerException
    {
        // and configured files via include/exclude and copy them into target dir
        for (String jspFileName : (List<String>)FileUtils.getFileNames(new File(webAppSourceDirectory), includes, excludes, false)) {
            // copy to target dir & add in target dir
            File sourceFile = new File(webAppSourceDirectory + "/" + jspFileName);
            File targetFile = new File(webAppTargetDirectory + "/" + jspFileName);
            getLog().debug("Copying " + sourceFile + " to " + targetFile);
            targetFile.getParentFile().mkdirs();
            IO.copy(sourceFile, targetFile);
            jspFilePaths.add(targetFile.getPath());
        }

        // parse web.xml
        Document doc = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(getWebXmlFile());
        Element root = doc.getDocumentElement();
        if (!root.getTagName().equals("web-app"))
            throw new IllegalArgumentException("'web-app' document expected in '" + webXml + "'");
        NodeList servletMappings = root.getElementsByTagName("servlet-mapping");
        for (int i = 0; i < servletMappings.getLength(); i++) {
            Element servletMapping = (Element)servletMappings.item(i);
            NodeList servletNames = servletMapping.getElementsByTagName("servlet-name");
            if (servletNames.getLength() != 1)
                continue;
            if (!servletNames.item(0).getTextContent().equals("jsp"))
                continue;
            NodeList urlPatterns = servletMapping.getElementsByTagName("url-pattern");
            for (int j = 0; j < urlPatterns.getLength(); j++) {
                String url = urlPatterns.item(j).getTextContent();
                File sourceFile = new File(webAppSourceDirectory + url);
                File targetFile = new File(webAppTargetDirectory + url);
                ...
                if (targetFile.exists())
                    ...
                    getLog().warn("Source for JSP file '" + url + "' is not found.");
            }
            root.removeChild(servletMapping);
        }

        // write modified web.xml document into a string
        StringWriter webXmlWriter = new StringWriter();
        Result output = new StreamResult(webXmlWriter);
        Source input = new DOMSource(doc);
        TransformerFactory.newInstance().newTransformer().transform(input, output);
        return webXmlWriter.toString();
    }
